...
!-------------------------------------------------------------------------------------------------------------------------------------------
  subroutine calculate_coriolis_term(params, mu, rho_grid, theta_grid)
    ...
    K_row = params % K(1)
    K_col = iff(params % fixed_basis % enabled == 1, K_row, K_row + 1)
...
!-------------------------------------------------------------------------------------------------------------------------------------------
  subroutine calculate_asym_term(params, K_row, K_col, mu, rho_grid, theta_grid)
    ...
    call assert(K_row + 2 == K_col .or. K_row == 1 .and. K_col == 1 .or. params % fixed_basis % enabled == 1 .and. K_row == K_col, 'Wrong combination of Ks for asym term')
    my_id = get_proc_id()
    n_procs = get_num_procs()
    n_basis = params % basis_size_phi
    ...
    do n = my_id + 1, size(rho_grid), n_procs
      ...
      if (K_row == 1 .and. K_col == 1 .and. params % fixed_basis % enabled == 0) then
...
!-------------------------------------------------------------------------------------------------------------------------------------------
  subroutine calculate_overlaps_extra(params, mu, rho_grid, theta_grid)
    ...
    ! coriolis term
    if (params % K(1) + 1 <= params % J .or. params % fixed_basis % enabled == 1) then
