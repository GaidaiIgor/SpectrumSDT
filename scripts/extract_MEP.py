...
Launch this script from within the folder with grid files and pes.out.
The result will be written to a file named MEP_rho.dat.
This file can be used to generate optimized rho-grid (see manual for details). """


def main():
    grid_sizes = [0]*3
    with open("grid_rho.dat") as grid_file:
        grid_sizes[0] = int(grid_file.readline().split()[3])
        grid_rho = [float(line.split()[0]) for line in grid_file]
    with open("grid_theta.dat") as grid_file:
        grid_sizes[1] = int(grid_file.readline().split()[3])
    with open("grid_phi.dat") as grid_file:
        grid_sizes[2] = int(grid_file.readline().split()[3])
    with open("pes.out") as potential_file:
