...
def processDir(petscdir, bfort, verbose, dirpath, dirnames, filenames):
  ...
  newls = [l for l in filenames if os.path.splitext(l)[1] in ['.c','.h','.cu']]
  if newls:
    PrepFtnDir(outdir)
    ...
    for i in range(0, len(newls), split_ct):
      cmd = 'BFORT_CONFIG_PATH='+os.path.join(petscdir,'lib','slepc','conf')+' '+bfort+' '+' '.join(options+newls[i:i+split_ct])
        ...
        for sfile in os.listdir(os.path.join(petscdir,basedir,submansec+'-tmpdir')):
          ...
          while txt:
            ...
            if txt.find('subroutine ') > -1:
              fd.write('      use slepc'+mansec+'def\n')
            txt = fdr.readline()
          fdr.close()
        fd.close()
        import shutil
        shutil.rmtree(os.path.join(basedir,submansec+'-tmpdir'))
